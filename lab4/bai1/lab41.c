...
static int __init init_chrdev(void)
{
	int ret = 0;
	vchrdev.dev_num = 0;
	ret = alloc_chrdev_region(&vchrdev.dev_num, 0, 1, name);
	if (ret < 0)
	{
	...
	vchrdev.dev_class = class_create(THIS_MODULE, "lab4_1");
	...
	vchrdev.dev = device_create(vchrdev.dev_class, NULL, vchrdev.dev_num, NULL, "lab4_1");
